...
export default function CursorSwitcher() {
  ...
  const springX = useSpring(mouseX, { stiffness: 280, damping: 32, mass: 0.5 });
  const springY = useSpring(mouseY, { stiffness: 280, damping: 32, mass: 0.5 });
  ...
  return (
    <div>
      ...
      {cursorType === "trail" && (
        <>
          <motion.div
            className="fixed z-[999] pointer-events-none w-2.5 h-2.5 rounded-full bg-amber-300 shadow-[0_0_10px_rgba(252,211,77,0.6)]"
            style={{
              left: springX,
              top: springY,
              translateX: "-50%",
      ...
      <motion.button
        ...
        className="fixed top-[22%] left-5 z-[1000] flex items-center justify-center w-12 h-12 rounded-full bg-[#0b0f2e]/90 backdrop-blur-md border border-white/10 shadow-lg hover:border-cyan-400/40 transition-colors duration-300"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="fixed top-[calc(22%+58px)] left-5 z-[1000] w-64 bg-[#0b0f2e]/95 backdrop-blur-md border border-white/10 rounded-2xl p-3 shadow-2xl font-poppins"
